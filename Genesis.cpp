  ...
  double* out2 = outputs[1];
  {
	  ...
	  if (targetLevel != currentLevel) {
		  ...
		  for (int s = 0; s < nFrames; ++s, ++in1, ++in2, ++out1, ++out2) {
			  if (abs(*in1) > 0.5 || abs(*in2)>0.5) {
				  *out1 = clipper1.process(lfo1.process(8 * filter17.process(filter8.process(filter3.process(filter2.process(filter1.process(((*in1) + (*in2)) / 4)))) + filter7.process(filter6.process(filter5.process(filter4.process(3 * (*in1 - *in2) / 4)))) + (*in1) / 16))*currentLevel);
				  *out2 = clipper2.process(lfo2.process(8 * filter18.process(filter16.process(filter11.process(filter10.process(filter9.process(((*in1) + (*in2)) / 4)))) + filter15.process(filter14.process(filter13.process(filter12.process(3 * (*in1 - *in2) / 4)))) + (*in2) / 16))*currentLevel);
			  }
			  else {
				  *out1 = clipper1.process(8 * filter17.process(filter8.process(filter3.process(filter2.process(filter1.process(((*in1) + (*in2)) / 4))) + (filter7.process(filter6.process(filter5.process(filter4.process(3 * (*in1 - *in2) / 4))))) + (*in1) / 16))*currentLevel);
				  *out2 = clipper2.process(8 * filter18.process(filter16.process(filter11.process(filter10.process(filter9.process(((*in1) + (*in2)) / 4))) + (filter15.process(filter14.process(filter13.process(filter12.process(3 * (*in1 - *in2) / 4))))) + (*in2) / 16))*currentLevel);
			  }
			  currentLevel += ramp;
		  }
	  }
